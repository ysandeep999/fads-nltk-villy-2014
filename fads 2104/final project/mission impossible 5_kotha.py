...
# FADS 2014 final project
# ACM Tramsaction on parallel computing
...
# Function takes paths of pdf files as input
def name_input():
    list_of_names = []
    no = eval((input("enter number of pdf's : ")))

    for n in range(no):
        path = input("enter path of pdf file include .pdf extension: ")
        list_of_names.append(path)

    return list_of_names


# This function merges bigrams and trigrams of all files
def merger(b_2,t_3):
...
def show_output_for_all_pdf(all_txt):
    ...
    li.append(("all pdf's",a,b))
    return show_output(li)
...
def get_freq_occur_words_with_val(common):

    temp_lst = []
    for tuples in common:
        temp_lst.append(tuples)
...
def mining_data(text): 
    ...
    bi_in_data = get_freq_occur_words_with_val(bi_common_tups)
    tri_in_data = get_freq_occur_words_with_val(tri_common_tups)
...
def main():
    ...
    #pdf_file_names1 = name_input()
    text_from_pdf = "bible_shakes.txt"

    for pdf_file in range(1):
        text_from_pdf = pdf_to_text(text_from_pdf)
        #all_txt = all_txt + "\n" + text_from_pdf
    ...
    show_output([["merged bigrams and trigrams",twos,threes]])
